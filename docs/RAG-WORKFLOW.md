...
The request signal is threaded route → runner → model stages → retriever. The runner checks it between stages and forwards it to AI SDK model calls. Pinecone SDK v6.1 does not accept an `AbortSignal` for the vector query, so the retriever checks immediately before the paid query and again before continuing; an already in-flight Pinecone inference/query cannot be forcibly interrupted by this code. Cancellation emits a terminal failed outcome with code `aborted` and does not start later stages.
